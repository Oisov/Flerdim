...
\begin{tikzpicture}[node distance=2cm]
  ...
  \node (start) [startstop,fill=blue!40!white] {$\displaystyle \iint_{S} \F \cdot \dSS$};
  \node (stop1) [startstop, right of=start, xshift=2cm, align=left] {$\iiint_{T} \div \F \dV$};
  \node (dec1) [decision, below of=stop1, yshift=-0.5cm, align=left] {$S$ closed?};
  \node (dec2) [decision, left of=dec1, xshift=-2cm, align=left] {$\F = \curl \G$?};
  \node (dec3) [decision, left of=dec2, xshift=-2cm, align=left] {$S \cup B$ closed?};
  \node (stop2) [startstop, below of=dec3, yshift=-0.5cm, xshift=1.43cm, align=left] {$\int_a^b\int_c^d \F(\rr(u,v))\cdot \left( \diffp{\rr}{u} \times \diffp{\rr}{v} \right)\mathrm{d}(u,v)$};
  \node (stop3) [startstop, above of=dec3, yshift=0.5cm, align=left] {$\int_{\partial S} \G \cdot \dr$};
  \node (stop4) [startstop, right of=stop2, xshift=4cm, align=left] {$\iiint_T \div \F \dV - \iint_B \F \dSS$};
